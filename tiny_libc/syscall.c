...
int sys_ps(char *buf) {
    return invoke_syscall(SYSCALL_PS, (long)buf, IGNORE, IGNORE, IGNORE, IGNORE);
...
void sys_task_set_p(pid_t pid, int mask, char* buf) {
    invoke_syscall(SYSCALL_TASK_SETP, pid, mask, (long)buf, IGNORE, IGNORE);
}
...
void pthread_create(pthread_t *thread, void (*start_routine)(void *), void *arg) {
    invoke_syscall(SYSCALL_THREAD_CREATE, (uintptr_t)thread, (uintptr_t)start_routine,
                   (uintptr_t)arg, IGNORE, IGNORE);
}
...
int sys_net_send(void *txpacket, int length) {
    ...
    return invoke_syscall(SYSCALL_NET_SEND, (long)txpacket, (uintptr_t)length, IGNORE, IGNORE,
                          IGNORE);
}

int sys_net_recv(void *rxbuffer, int pkt_num, int *pkt_lens) {
    /* TODO: [p5-task2] call invoke_syscall to implement sys_net_recv */
    return invoke_syscall(SYSCALL_NET_RECV, (long)rxbuffer, (uintptr_t)pkt_num, (long)pkt_lens,
                          IGNORE, IGNORE);
}

int sys_net_recv_stream(void *buffer, int *nbytes) {
    return invoke_syscall(SYSCALL_NET_RECV_STREAM, (long)buffer, (long)nbytes, IGNORE, IGNORE,
                          IGNORE);
}
