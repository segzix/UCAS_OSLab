...
int spin_lock_try_acquire(spin_lock_t *lock) {
    return atomic_swap(LOCKED, (ptr_t)(lock_status_t *)lock);
}
...
void spin_lock_release(spin_lock_t *lock) {
    atomic_swap(UNLOCKED, (ptr_t)(lock_status_t *)lock);
    return;
}
...
void do_barrier_wait(int bar_idx) {
    ...
    if (barrier_now->queue_size < barrier_now->target_size)
        do_block(&get_pcb()->list, &barrier_now->barrier_queue, &barrier_now->lock);
...
void do_semaphore_down(int sema_idx) {
    ...
    semaphore_now->semaphore_size--;
    if (semaphore_now->semaphore_size < 0)
        do_block(&get_pcb()->list, &semaphore_now->semaphore_queue, &semaphore_now->lock);
...
int do_mbox_send(int mbox_idx, void *msg, int msg_length) {
    ...
    while (msg_length + mailbox_now->siz > MAX_MBOX_LENGTH) { //生产者发现当前空余量已经不够生产
        ...
        printl("\nadd (%s %d) to mboxs[%d] (%s) send_queue\n", current_running->pcb_name,
               current_running->pid, mbox_idx, mailbox_now->name);
    ...
    if (msg_length + mailbox_now->tail <= MAX_MBOX_LENGTH) { //如果发现不用循环，直接往后加即可
        ...
        // printl("send buffer=%s,msg=%s,len=%d",msg,((*mailbox_now).buffer +
        // mailbox_now->tail),msg_length);
    } else {
    ...
    while (mailbox_now->mailbox_recv_queue.next !=
           &(mailbox_now->mailbox_recv_queue)) //生产成功唤醒所有消费者
...
int do_mbox_recv(int mbox_idx, void *msg, int msg_length) {
    ...
    mailbox_now->siz -= msg_length;
    if (mailbox_now->head + msg_length <=
        MAX_MBOX_LENGTH) { //消费是改变头指针，消费完之后看是否需要掉头循环
        // printl("recv buffer=%s,msg=%s,len=%d",msg,((*mailbox_now).buffer +
        // mailbox_now->head),msg_length);
    ...
    while (mailbox_now->mailbox_send_queue.next !=
           &(mailbox_now->mailbox_send_queue)) //消费成功唤醒所有生产者
