...
char inode_block
    [BLOCK_SIZ]; //每次读出的关于inode，都将先存放在这个block中，并且返回这个block中对应的inode指针
int judge;
...
uint8_t *bread(uint32_t block_id) { //选择要读的硬盘数据块号，读出并且返回读到的bcache地址
    ...
    //如果有，那么直接从bcache中读
    bios_sd_read(kva2pa((uintptr_t)bcaches[bcache_point].bcache_block), 8,
                 blockid2sectorid(block_id));
...
uint8_t *balloc(
    uint32_t block_id) { //分配一块bcache空间并进行修改，注意这个时候是先修改然后调用bwrite来返回
...
int32_t allocbit(
    char *
        count_block) { //根据已有的count_block来读出其中还没有写进的bit位并返回对应偏移实现分配,同时也会将这一个bit位拉高
    uint32_t cnt = 0;
...
uint32_t alloc_block(int zero, int64_t bcache_offset) {
    ...
    if (bcache_offset != -1) {
        ...
        block_indirect_offset = ((uint8_t *)bcache_offset - (uint8_t *)bcaches) % sizeof(bcache_t) -
                                8; //这里应该是对应的bcache_block中的偏移
...
int free_inodeblock(inode_t *inode) { //将一个inode所有的block块全部给清空
    ...
    if (inode->indirect_1) { //第一层
        uint32_t *indirect1map =
            (uint32_t *)bread(inode->indirect_1); //这里读的不是blockbitmap，而是数据块中的间指块
        for (uint32_t i = 0; i < BLOCK_SIZ / 4; i++) {

            if (indirect1map[i]) { //第二层
                blockbitmap =
                    bread(superblock->blockbitmap_offset + indirect1map[i] / BLOCK_BIT_SIZ);
                freebit(blockbitmap, indirect1map[i] % BLOCK_BIT_SIZ);
                bwrite(superblock->blockbitmap_offset + indirect1map[i] / BLOCK_BIT_SIZ,
                       blockbitmap);
            }
        }
            ...
            if (indirect2map1[i]) { //第二层
                ...
                for (uint32_t j = 0; j < BLOCK_SIZ / 4; j++) {
                    ...
                    if (indirect2map2[j]) { //第三层
                        blockbitmap = bread(superblock->blockbitmap_offset +
                                            indirect2map2[j] / BLOCK_BIT_SIZ);
                        freebit(blockbitmap, indirect2map2[j] % BLOCK_BIT_SIZ);
                        bwrite(superblock->blockbitmap_offset + indirect2map2[j] / BLOCK_BIT_SIZ,
                               blockbitmap);
                    }
                }
                blockbitmap =
                    bread(superblock->blockbitmap_offset + indirect2map1[i] / BLOCK_BIT_SIZ);
                freebit(blockbitmap, indirect2map1[i] % BLOCK_BIT_SIZ);
                bwrite(superblock->blockbitmap_offset + indirect2map1[i] / BLOCK_BIT_SIZ,
                       blockbitmap);
            }
        }
        //如果有间接指针，那么必定要全部free掉
...
int rmfile(uint32_t ino) { //这个函数用于删除一个目录或者文件
    ...
    if ((inode->mode == INODE_DIR) &&
        inode->filesz > 0x40) { //是目录项并且有表项//表项当然不能是.和..!!
        dentry = (dentry_t *)search_datapoint(inode, dentryoffset);
        while (dentry->dentry_valid &&
               (dentryoffset < inode->filesz)) { //注意不能比filesz大的部分还找
            rmfile(dentry->ino);
            dentryoffset += 32;
            dentry = (dentry_t *)search_datapoint(inode, dentryoffset);
        }
    ...
    if (inode->mode == INODE_FILE && ((int)inode->fd_index != 0xff) &&
        !inode->hardlinks) //必须要满足没有硬链接才释放
        fdescs[inode->fd_index].used = 0;

    if ((inode->mode == INODE_FILE) &&
        (inode->hardlinks != 0)) //当为文件并且硬链接不为0时不能释放相应数据块，其他情况均应释放
        inode->hardlinks--;
    else {
...
uint8_t *search_datapoint(
    inode_t *inode,
    uint32_t
        offset) { //根据给出的偏移，直接返回对应的数据块中的地址，同时注意如果发现中间有映射没有建立好，那么同样的会把所有映射建立好
    uint8_t *ret_point;
    ...
    if (offset < DIRECT_BLOCK_SIZ * BLOCK_SIZ) { //标号位于直接块内
        ...
        uint32_t *point = (uint32_t *)indirect1_point + (offset - DIRECT_BLOCK_SIZ * BLOCK_SIZ) /
                                                            BLOCK_SIZ; // point块，还不是目录块
        ...
        uint32_t *point1 = (uint32_t *)indirect2_point1 +
                           (offset - INDIRECT1_BLOCK_SIZ * BLOCK_SIZ) /
                               (BLOCK_SIZ * POINT_PER_BLOCK); // point块，还不是目录块
        uint32_t temp_point1 = *point1;

        if (!(*point1)) //说明还没有进行过对应数据块的分配
            temp_point1 =
                alloc_block(0, (int64_t)point1); //分配，分配的同时,point指针出的blockid号也已经置好
        //如果是现在才分配，那么point1指向的blockid会返回，但是由于可能会被覆盖掉，因此不能采用之前的写法

        uint8_t *indirect2_point2 = (uint8_t *)bread(temp_point1);
        uint32_t *point2 =
            (uint32_t *)indirect2_point2 +
            ((offset - INDIRECT1_BLOCK_SIZ * BLOCK_SIZ) % (BLOCK_SIZ * POINT_PER_BLOCK)) /
                BLOCK_SIZ; // point块，还不是目录块
...
void bigfile_alloc(
    inode_t *inode,
    uint32_t
        offset) { //与searchdatapoint基本一致，唯一的不同在于这个时候block肯定是没有被分配过的，因此分配就行了不用读
...
int write_file(inode_t *inode, char *string, uint32_t start,
               uint32_t length) { // inode指定了文件，string指定内容，其他指定起始位置和长度
    ...
    inode_t buff_inode;
    uint32_t buff_ino =
        ((((uint8_t *)inode - (uint8_t *)bcaches) % sizeof(bcache_t)) - 8) / sizeof(inode_t);
    ...
    while (length) {
        ...
        if (buff_inode.mode == INODE_FILE && ((int)buff_inode.fd_index != 0xff))
            fdescs[buff_inode.fd_index].memsiz +=
                seg_length; //中间你写是覆盖空洞还是什么我不管，只要你写了我就加
    }
    ...
    bios_sd_read(kva2pa((uintptr_t)check_block), 8,
                 blockid2sectorid(bcaches[bcache_inode_index].block_id)); //测试
    //写文件之后inode必然会被修改，因此必须落盘

    return 1;
}

int read_file(inode_t *inode, char *string, uint32_t start,
              uint32_t length) { // inode指定了文件，string指定内容，其他指定起始位置和长度
    uint8_t *bcache_start;
    uint8_t *bcache_end;
    uint32_t bcache_index;
...
dentry_t *name_search_dentry(
    inode_t *inode_current,
    char *name) { //对于目录项而言，给一个当前目录级的名字，可以找到其在数据块中的具体位置并返回地址
    dentry_t *dentry;
    ...
    while (dentry->dentry_valid &&
           (dentry_offset < inode_current->filesz)) { //没有到尽头//同时还要注意不能够比filesz大
...
int inopath2ino(
    uint32_t base_ino,
    char *dir_name) { //根据给出的目录和路径，返回对应文件或目录的ino号；因为最终要做的时更新pwd
