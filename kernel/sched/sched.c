...
void do_thread_create(pid_t *thread, void *thread_entrypoint, void *arg) {
    ...
    for (id = 0; id < NUM_MAX_TASK; id++) {
        if (pcb[id].status == TASK_EXITED) {
            ...
            pcb[id].kernel_sp =
                kalloc() +
                1 * PAGE_SIZE; //这里的给出的用户映射的虚地址没有任何意义 pcb[id].user_sp    =
            USER_STACK_ADDR + 2 * PAGE_SIZE *pcb[id].tid; //必须是分配两页用户栈

            kva_user_stack =
                uvmalloc(pcb[id].user_sp - PAGE_SIZE, pcb[id].pgdir,
                         _PAGE_PRESENT | _PAGE_READ | _PAGE_WRITE | _PAGE_EXEC | _PAGE_USER) +
                1 * PAGE_SIZE; //比栈地址低的一张物理页
            // uintptr_t va = alloc_page_helper(pcb[id].user_sp - PAGE_SIZE, pcb[id].pgdir) +
            // PAGE_SIZE;
            ...
            init_tcb_stack(pcb[id].kernel_sp, pcb[id].user_sp, entry_point, &pcb[id],
...
int do_process_show() {
    ...
    while (i < NUM_MAX_TASK) {
        switch (pcb[i].status) {
        case TASK_RUNNING:
            printk("[%d] NAME : %s  PID : %d TID : %d TRUEPID : %d STATUS : TASK_RUNNING ", i,
                   pcb[i].pcb_name, pcb[i].pid, pcb[i].tid, pcb[i].truepid);
