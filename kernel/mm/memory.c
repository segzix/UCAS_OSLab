...
pid_t do_fork() {
    ...
    for (int id = 0; id < NUM_MAX_TASK; id++) {
        if (pcb[id].status == TASK_EXITED) {
            ...
            pcb[id].recycle = 0;
            pcb[id].pgdir =
                (PTE *)kalloc(); //分配根目录页//这里的给出的用户映射的虚地址没有任何意义
            ...
            // kva_user_stack = alloc_page_helper(pcb[id].user_sp - PAGE_SIZE, pcb[id].pgdir,1,id+2)
            // + 1 * PAGE_SIZE;//比栈地址低的一张物理页 alloc_page_helper(pcb[id].user_sp -
            // 2*PAGE_SIZE, pcb[id].pgdir,1,id+2);//比栈地址低的第二张物理页 uintptr_t va =
            // alloc_page_helper(pcb[id].user_sp - PAGE_SIZE, pcb[id].pgdir) + PAGE_SIZE;
            ...
            regs_context_t *pt_regs =
                (regs_context_t *)(pcb[id].kernel_sp - sizeof(regs_context_t));

            uintptr_t src_kernel_sp_start =
                (*current_running)->kernel_sp -
                sizeof(regs_context_t); //这个时候kernel_sp只减去了pt_regs一段
            uintptr_t dest_kernel_sp_start = (uintptr_t)pt_regs;

            memcpy((void *)dest_kernel_sp_start, (void *)src_kernel_sp_start,
