...
uint64_t load_task_img(int taskid,uintptr_t pgdir,int pid)
{
    ...
    memcpy(load_entrypoint_kva,load_entrypoint_kva + task_block_offset,tasks[taskid].task_block_size);
    ...
    for(uint32_t i = 0; i < task_block_num; i += (NORMAL_PAGE_SIZE >> 9),page_number++){
        ...
        memcpy(va,pa2kva(load_entrypoint + page_number * NORMAL_PAGE_SIZE),NORMAL_PAGE_SIZE);
    }
    //根据它的扇区数可以推断出占几个页
